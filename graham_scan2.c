...
Coord* grahamScan2(Coord points[], int n, int* hullSize) {
    int i, x; // Declare loop variable here to avoid re-declaration errors
    ...
    //Find the anchor point with the lowest y-coordinate (leftmost if tied)
    int minY = 0;
    for (i = 1; i < n; i++) {
    ...
    //Sort the points by polar angle with respect to the anchor using heap sort
    ...
    //Initialize the stack and push the first three points
    Stack s;
    createStack(&s, n);
    push(&s, points[0]);
    push(&s, points[1]);
